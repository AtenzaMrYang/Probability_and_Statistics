...
\begin{solution}
    ...
    $$
        F(x) = \int_{-\infty}^{0.3} 0\,\mathrm{d}x + \int_{0.3}^x 5(1-t)\,\mathrm{d}t
        = \left.\left(-\frac{5}{2}t^2 + 5t\right)\right|_{0.3}^{x}
        = -\frac{5}{2}x^2 + 5x - \frac{51}{40}.
    $$
    \paragraph{情形三} 当 $x \geqslant 0.7$ 时
    $$
        F(x) = \int_{-\infty}^{0.3}0\,\mathrm{d}t + \int_{0.3}^{0.7}5(1-t)\,\mathrm{d}t + \int_{0.7}^{x} 0\,\mathrm{d}t = 1.
    $$
    综上所述，分布函数 $F(x)$ 为
    $$
        F(x) = \int_{-\infty}^x f(t) \,\mathrm{d}t
